...
class batch_temp(object):
    IS_BATCH = {}
    # Store last processing time for each user to implement delay
    LAST_PROCESS_TIME = {}
...
async def save(client: Client, message: Message):
    if "https://t.me/" in message.text:
        ...
        for msgid in range(fromID, toID + 1):
            ...
            # Check if we need to wait before processing next file
            last_time = batch_temp.LAST_PROCESS_TIME.get(message.from_user.id)
            if last_time:
                time_since_last = time.time() - last_time
                if time_since_last < 30:  # 30 seconds delay between files
                    wait_time = 30 - int(time_since_last)
                    if wait_time > 0:
                        # Send a status message about waiting
                        wait_msg = await client.send_message(
                            message.chat.id,
                            f"⏳ **Waiting {wait_time} seconds before next file to prevent account ban...**"
                        )
                        await asyncio.sleep(wait_time)
                        await wait_msg.delete()
            
            # Update last processing time
            batch_temp.LAST_PROCESS_TIME[message.from_user.id] = time.time()
